...
parser.add_argument("--server", "-s", required=True, help="Telner server IP address (e.g. -s 192.168.1.115)") # Telnet server IP address
parser.add_argument("--port", "-p", help="Telner server port (e.g. -p 23), if not supplied port 23 will be utilised by default") # Telnet server port
parser.add_argument("--log", action='store_true', help="Listen and log all key strokes from and to server") # Monitor and log session
parser.add_argument("--hijack", help="Hijack telnet server and issue command (e.g. --hijack touch test.txt)") # Hijack and command inject session
